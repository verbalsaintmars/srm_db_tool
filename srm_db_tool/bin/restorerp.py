...
if result.site == "both":
    pp_msg = "Choose to restore recovery plan on both site, " +\
             "but no complete Protected Site DB " +\
             "Connection information provided."
    ss_msg = "Choose to restore recovery plan on both site, " +\
...
restorerp = RestoreRecoveryPlan(pp_conn, ss_conn, tableOp)

if result.site == 'both':
    restorerp(result.rp_name, a_site='pp')
    restorerp(result.rp_name, a_site='ss')
